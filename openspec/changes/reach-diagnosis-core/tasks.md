...
| 11 | `rules.go` (additions), `matrix_test.go`, `rules_test.go` (additions) | R-HR-03, PRD §13, §1.1, RG-2, design §5.2 | 410–610 (≈510) | Medium | `go test ./internal/diagnosis/ -run 'TestMatrixReplay\|TestSSHDestination\|TestCloudflare'` | The six literal §1.1 rows with their counterfactuals, and the four edge states plus four HTTP/2 advice states, are two readable tables | Revert this PR; the mechanism and the other rule groups stay |
...
| 14 | `tailscale.go`, `notimplemented_test.go`, `cloudflare.go`, `registry_test.go` | RG-9, R-HR-06, PRD §5.2, §14.5 | 380–570 (≈475) | Low | `go test ./internal/transport/ -run 'TestNotImplemented\|TestRegistry\|TestCloudflareTunnel'` | Every registered adapter's three members are proven to fail loudly and return no value; exactly four transports and the unsatisfied hostname row are one file each | Delete the four files; the registry returns to two adapters |
...
| 18 | `internal/doctor/exit.go`, `doctor.go`, `flags.go`, `doctor_test.go`, `flags_test.go`, `docs/diagnosis-report.md` (one clarifying line) | R-HR-07, R-HR-NF-02, R-HR-30, RG-13, cli-report hub, design §3.4 | 430–650 (≈540) | Medium | `go test ./internal/doctor/ -run 'TestExitMatrix\|TestDocMatchesExitCodes'`; `go test ./internal/doctor/ -run TestFlags` | The whole exit matrix runs over scripted seams with no CLI binary and no real network; every parse rule and usage error is one table that starts no measurement | Delete the five files; the probe and report layers stay |
...
`Files`: `internal/diagnosis/{ids.go,rules.go,findings.go,rules_test.go,unresolved_test.go,agnostic_test.go}`. `Depends on`: PR 11.
Adjudicated 2026-09-19 (design §5.2's dated note): the `tls.*` groups are restated to their derived ids, `Need` gains an optional observation label for `local.sshd`, and `NODE_WSL2_SYSTEMD_ABSENT` is deferred to the slice that carries the service-manager signal structurally. `ids.go` and `findings.go` are disclosed enabling edits, as in PR 11: the hand-named vocabulary and every conclusion text have their single home there.
Second-split boundary applied in review: the rule groups landed first (WU21) and the two suites second (WU22), as two commits on one branch.

- [x] **RED** — add one case per adjudicated id: the derived `TLS_INTERCEPTION_{PASS,FAIL,UNRESOLVED,NOT_MEASURED}` and `TLS_TRUSTSTORE_{PASS,FAIL,UNRESOLVED,NOT_MEASURED}` (with the RG-3 platform/override assertions), `SSHD_PRESENT_CONFIG_DIVERGENT`, `SSHD_ABSENT`, `SSHD_EFFECTIVE_CONFIG_NOT_MEASURED`, `SSHD_PRESENT_CONFIGURED`, `NODE_PLATFORM_REFUSED_NATIVE_WINDOWS`, `NODE_PLATFORM_UNKNOWN`, `NODE_PLATFORM_SUPPORTED`, plus `LOCAL_ENV_*`; and assert `AllRuleIDs()` declares none of §5.2's hand-named tls spellings and no `NODE_WSL2_SYSTEMD_ABSENT`. <!-- sdd-owner: implementation -->
- [x] **GREEN** — implement `Need.Label`, the `local.sshd` and `node.platform` groups in `rules.go` and their conclusion wording in `findings.go`, with `SSHD_EFFECTIVE_CONFIG_NOT_MEASURED` as the default live case and no transport viability derived here. No production code is added for the restated tls groups: their derived fact rules already carry the conclusions. <!-- sdd-owner: implementation -->
- [x] **TRIANGULATE** — the RG-3 platform limitation reaches a conclusion text through `TLS_TRUSTSTORE_UNRESOLVED`, a diverging config never fires `SSHD_PRESENT_CONFIGURED`, the deny-all default case fires `SSHD_EFFECTIVE_CONFIG_NOT_MEASURED`, and a stopped service keeps its own fact while never being reported as an absent binary. <!-- sdd-owner: implementation -->
- [x] **REFACTOR + GATE** — `gofmt -l .`, `go vet ./...`, `go test ./internal/diagnosis/`. <!-- sdd-owner: implementation -->
- [x] **RED** — write `unresolved_test.go`: a rule needing `measured/pass` or `measured/fail` is unreachable on unresolved or not-measured input, the weaker rule fires and names the unresolved probe, and independent conclusions are byte-identical to the run without the unresolved probe (R-HR-NF-03). <!-- sdd-owner: implementation -->
- [x] **GREEN** — implement the structural matching that makes the confident rule unreachable plus the completeness computation over observations by resolution (never over verdicts), so no fall-through default is needed. <!-- sdd-owner: implementation -->
- [x] **TRIANGULATE** — add the obligation 1/2 case: a not-measured observation produces a not-measured conclusion naming what was missing, an attempted-but-unresolved observation produces an unresolved conclusion naming the probe, and NF-03 holds at the engine layer. <!-- sdd-owner: implementation -->
- [x] **RED + GREEN** — write `agnostic_test.go`, which reads the `diagnosis` package's own non-test sources, strips the registered probe names, and asserts none of `direct-ssh`, `reverse-ssh`, `cloudflare-tunnel`, `tailscale`, `argotunnel`, `7844` remain, with the three controls that make the guard falsifiable (NF-04, design §6.4). <!-- sdd-owner: implementation -->
- [x] **REFACTOR + GATE** — `gofmt -l .`, `go vet ./...`, `go test ./internal/diagnosis/`. <!-- sdd-owner: implementation -->
...
- [x] **REFACTOR + GATE** — `gofmt -l .`, `go vet ./...`, `go test ./internal/transport/ -run 'TestRegistry|TestCloudflareTunnel'`. <!-- sdd-owner: implementation -->

### PR 15 — WU27 + WU28 · Recommendation wording, the pin note and the feasibility evidence table

`Files`: `internal/transport/cloudflare.go` (advice block), `internal/transport/{pin_note.go,pin_test.go,feasibility_test.go,testtransport_test.go}`, and `internal/transport/contract_test.go` for the consolidation this slice forces (the candidate-only fixture moves to its spec'd home, `testtransport_test.go`). `Depends on`: PR 14.
Decisions recorded 2026-09-20 (design §3.2's PR 15 note): the pin statement rides every `cloudflare-tunnel` row's notes regardless of edge state or advice, and the one-home proof asserts the statement's exact text occurs exactly once in the code and shipped docs — in `pin_note.go` — because `README.md` and `PRD.md` carry the project's prose about the upstream report rather than the tool's statement.
...
- [x] **RED** — write `pin_test.go`: the pin statement names upstream issue #1673 with its state (open, uncommented), names version 2026.6.0 **only**, states that later releases' behaviour is unknown, cites release 2026.5.1's published SHA256 checksums, asserts no version-range claim and no claim of installing, verifying or pinning anything on the machine, and asserts exactly one home for the wording (RG-1, R-HR-16). <!-- sdd-owner: implementation -->
- [x] **GREEN** — implement `pin_note.go` as the single home and the QUIC-failed/unconfirmed/usable/not-assessed advice in `cloudflare.go`. <!-- sdd-owner: implementation -->
- [x] **TRIANGULATE** — assert the advice states that HTTP/2 forfeits post-quantum key agreement and that this slice recommends and does not enforce; assert no output claims a fallback was applied or a configuration written; assert working QUIC produces no downgrade advice (R-HR-05, RG-6). <!-- sdd-owner: implementation -->
- [x] **REFACTOR + GATE** — `gofmt -l .`, `go vet ./...`, `go test ./internal/transport/`. <!-- sdd-owner: implementation -->
- [x] **RED** — write `feasibility_test.go`: a finding-set × adapter table covering viable, non-viable, detected-but-blocked and unmeasured-dependency cases; for the PRD §1.1 replay the direct-SSH and reverse-SSH reasons name the blocked hub target with its port; no reason asserts a measurement that was never made (R-HR-06). <!-- sdd-owner: implementation -->
- [x] **GREEN** — extract the shared feasibility helpers the table exposes, without introducing a per-transport branch into `diagnosis`. <!-- sdd-owner: implementation -->
- [x] **TRIANGULATE** — write `testtransport_test.go`: register a `testTunnel` implementing only `Candidate`, assert the feasibility list contains it with its stated viability and reason, and assert every `diagnosis` finding is byte-identical to the same run without it (NF-04, design §6.4). <!-- sdd-owner: implementation -->
- [x] **REFACTOR + GATE** — `gofmt -l .`, `go vet ./...`, `go test ./...` and confirm the agnostic guard still passes with all four adapters present. <!-- sdd-owner: implementation -->
...
`Files`: `internal/doctor/{exit.go,doctor.go,flags.go,doctor_test.go,flags_test.go}` — the flag surface's home is the doctor package, not `cmd/herdr-reach/`, because a `package main` without `main.go` fails `go build ./...` (design §7's PR 18 note) — plus one clarifying line in `docs/diagnosis-report.md`'s Streams section. `Depends on`: PR 17.
Decision recorded 2026-09-20 (design §7's PR 18 note): the command line is `herdr-reach doctor [flags]`, with the subcommand owned by the flag parser; `--version` writes to standard output and `-h`/`--help` to standard error, both exiting `0`; and the document's exit-code table is asserted against the constants here, in `doctor_test.go`.
...
- [x] **RED** — write the exit matrix in `doctor_test.go` over scripted seams: `0` for a completed run including a negative answer, no viable transport and a native-Windows refusal; `1` when at least one probe was attempted and resolved unresolved (including the hanging probe); `2` for an unknown flag, an unusable flag value and an internal error with no diagnosis presented as completed; a not-measured probe alone leaves the exit code unchanged (R-HR-07, R-HR-30). <!-- sdd-owner: implementation -->
- [x] **GREEN** — implement `exit.go` (`ExitOK`, `ExitIncomplete`, `ExitUsage`) and `doctor.go`: `EffectiveTargets` → `Registry` → runner → `diagnosis.Diagnose` → `transport.Evaluate` → `report.Build` → `WriteHuman(stderr)` always, `WriteJSON(stdout)` only with `--json`. <!-- sdd-owner: implementation -->
- [x] **TRIANGULATE** — assert the exit-code constants ↔ `docs/diagnosis-report.md` table, and that `run.completeness` and the exit code agree in every matrix case. <!-- sdd-owner: implementation -->
- [x] **REFACTOR + GATE** — `gofmt -l .`, `go vet ./...`, `go test ./internal/doctor/`. <!-- sdd-owner: implementation -->
- [x] **RED** — write `flags_test.go`: `--json`, `--hub`, repeatable `--target`, `--version` and `-h` parse into `doctor.Options`; the default hub port `22` applies when the port is omitted; a bracketed IPv6 literal is accepted as host-only; an empty host, an unparsable port, an unknown `--target` probe name and a malformed address are usage errors. <!-- sdd-owner: implementation -->
- [x] **GREEN** — implement `flags.go` with the D3 parse rules; unusable input returns a usage error and starts no measurement run. <!-- sdd-owner: implementation -->
- [x] **TRIANGULATE** — add the hub-resolution cases: a supplied `host:port` becomes the hub target exactly and the resolved target is stated in the output; an omitted port resolves to 22 by the one documented rule; no hub yields not-measured/`input_missing_hub`, never "blocked", and no transport is viable on the strength of hub reachability (RG-13, R-HR-02). <!-- sdd-owner: implementation -->
- [x] **REFACTOR + GATE** — `gofmt -l .`, `go vet ./...`, `go test ./internal/doctor/`. <!-- sdd-owner: implementation -->
...
`Files`: `cmd/herdr-reach/main_test.go` (the writes-nothing proof, which the design had placed in the doctor package; `run` is a `package main` and cannot be imported), `internal/doctor/guard_test.go`, `internal/doctor/doctor_test.go` — the flag surface's file list is unchanged. `Depends on`: PR 19.
...
- [x] **RED** — run the CLI in-process with `HOME` and the working directory pointed at two fresh `t.TempDir()` trees, record recursive digests before and after, and assert byte-identical trees, no command-runner calls in the production shape (a wired denying runner's attempts are denied and pinned by their own case), and set equality between dialed `(host, port, protocol)` triples and `probe.EffectiveTargets(opts)` (R-HR-02, R-HR-24, R-HR-27, design §6.3). <!-- sdd-owner: implementation -->
- [x] **GREEN** — fix only inside the named files if the guard fails; never by weakening an assertion. <!-- sdd-owner: implementation -->
- [x] **TRIANGULATE** — add the "a write attempt cannot pass the guard" counter-case: a deliberate in-test write to the temp tree must fail the digest assertion. <!-- sdd-owner: implementation -->
- [x] **TRIANGULATE** — complete the three-level no-egress proof: sentinel reachability re-asserted from the doctor side (level 1) and the closed dialed set measured by a recording dialer built on the deny-all base (level 2). <!-- sdd-owner: implementation -->
- [x] State the coverage boundary in the test comment: a temp-`HOME` digest cannot prove the absence of writes outside `HOME`/CWD; the slice closes that by construction (no writer code path, no write-capable dependency) and the residual gap stays in the risk register (RG-14). <!-- sdd-owner: implementation -->
- [x] **RED** — add the PRD §1.1 replay through the real pipeline: it exits `0`, and both the human and machine projections carry the hub target string, the verbatim failing detail and the stable reason code (PRD §13, §1.1, RG-2). <!-- sdd-owner: implementation -->
- [x] **RED + GREEN** — add `TestForbiddenStringsAbsent` over one complete run's two projections: the WSL2 text contains the documented millisecond semantics and neither the child-of-init rule nor a `-1` sentinel (RG-4); no output claims an HTTP/2 fallback was applied or enforced (RG-6). Fix only inside the owning units' files. <!-- sdd-owner: implementation -->
- [x] **TRIANGULATE** — assert a complete run finishes inside an injected millisecond run budget with every probe bounded, and that a hanging-probe variant makes the run incomplete with exit `1` in both projections (R-HR-NF-02, R-HR-NF-09). <!-- sdd-owner: implementation -->
- [x] **REFACTOR + FINAL GATE** — run `gofmt -l .`, `go vet ./...`, `go test ./...` and a hand `go test -race ./...`; record the exact commands and results as the unit's evidence; confirm no linter was introduced (`quality.lint` stays empty). <!-- sdd-owner: implementation -->
- [x] Record the verify-phase hand-run obligations that no unit test can discharge: the §1.1 matrix reproduced by hand on the motivating network, the D8 raw-UDP result compared with `cloudflared`'s QUIC log lines, and the macOS trust-store behaviour on a real Mac. <!-- sdd-owner: implementation -->
